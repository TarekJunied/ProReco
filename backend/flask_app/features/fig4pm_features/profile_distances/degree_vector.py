...
def degree_profile_distance_all_variants(log, distance_measure):
    ...
    progress = tqdm(total=no_trace, leave=False,
                    desc="Degree profile distance all variants, completed :: ")
    ...
    progress.close()
    del progress
    return np.asarray(distance_per_variant_list, dtype=object), sum(distance_per_variant_list) / (total_number_of_trace_classes(log) ** 2)
...
def degree_profile_distance_all_cases(log, distance_measure):
    ...
    progress = tqdm(total=no_trace, leave=False,
                    desc="Degree profile distance all cases, completed :: ")
    ...
    progress.close()
    del progress
    return np.asarray(distance_per_trace_list, dtype=object), sum(distance_per_trace_list) / (total_number_of_traces(log) ** 2)
...
def out_degree_profile_distance_all_variants(log, distance_measure):
    ...
    no_trace = total_number_of_trace_classes(log)
    progress = tqdm(total=no_trace, leave=False,
                    desc="Out-degree profile distance all variants, completed :: ")
...
def out_degree_profile_distance_all_cases(log, distance_measure):
    ...
    no_trace = len(log)  # count number of traces and setup progress bar
    progress = tqdm(total=no_trace, leave=False,
                    desc="Out-degree profile distance all cases, completed :: ")
