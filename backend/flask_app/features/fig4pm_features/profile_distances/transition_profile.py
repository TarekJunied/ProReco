...
def transition_profile_distance_all_variants(log, profile, distance_measure):
    ...
    progress = tqdm(total=no_trace, leave=False,
                    desc="Transition profile distance all variants, completed :: ")
...
def transition_profile_distance_all_cases(log, profile, distance_measure):
    ...
    progress = tqdm(total=no_trace, leave=False,
                    desc="Transition profile distance all cases, completed :: ")
...
def markov_chain_transition_profile_distance_all_variants(log, profile, distance_measure):
    ...
    no_trace = total_number_of_trace_classes(log)
    progress = tqdm(total=no_trace, leave=False,
                    desc="Markov chain transition profile distance all variants, completed :: ")

    # Calculate distance for every variant, represented as a vector or a matrix
    if profile == 'matrix':
        encoding = markov_chain_transition_profile_encoding_all_variants_matrix(
            log)
        distance_per_variant_list = []
        if distance_measure == "cosine":
            for reference_var in encoding:
        ...
        encoding = markov_chain_transition_profile_encoding_all_variants_vector(
            log)
        distance_per_variant_list = []
        if distance_measure == "cosine":
            for reference_var in encoding:
        ...
        encoding = markov_chain_transition_profile_encoding_all_variants_vector_mean(
            log)
        distance_per_variant_list = []
        if distance_measure == "cosine":
            for reference_var in encoding:
...
def markov_chain_transition_profile_distance_all_cases(log, profile, distance_measure):
    ...
    no_trace = len(log)  # count number of traces and setup progress bar
    progress = tqdm(total=no_trace, leave=False,
                    desc="Markov chain transition profile distance all cases, completed :: ")

    # Calculate distance for every case, represented as a vector or a matrix
    if profile == 'matrix':
        encoding = markov_chain_transition_profile_encoding_all_traces_matrix(
            log)
        distance_per_trace_list = []
        if distance_measure == "cosine":
            for reference_trace in encoding:
        ...
        encoding = markov_chain_transition_profile_encoding_all_traces_vector(
            log)
        distance_per_trace_list = []
        if distance_measure == "cosine":
            for reference_trace in encoding:
        ...
        encoding = markov_chain_transition_profile_encoding_all_traces_vector_mean(
            log)
        distance_per_trace_list = []
        if distance_measure == "cosine":
            for reference_trace in encoding:
