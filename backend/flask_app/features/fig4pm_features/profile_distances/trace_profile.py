...
def trace_distance_all_variants(log):
    ...
    progress = tqdm(total=no_trace, leave=False,
                    desc="Trace distance all variants, completed :: ")
    ...
    for reference_var in variant_list:
        progress.update()
        dist = sum(distance(reference_var, compared_var)
                   for compared_var in variant_list)
...
def trace_distance_all_cases(log):
    ...
    progress = tqdm(total=no_trace, leave=False,
                    desc="trace distance all cases, completed :: ")
    ...
    for reference_trace in case_list:
        progress.update()
        dist = sum(distance(reference_trace, compared_trace)
    ...
    progress.close()
    del progress
    return np.asarray(distance_per_case_list, dtype=object), sum(distance_per_case_list) / (total_number_of_traces(log) ** 2)
