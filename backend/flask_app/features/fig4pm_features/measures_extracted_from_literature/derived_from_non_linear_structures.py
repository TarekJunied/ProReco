...
def graph_diameter(log, threshold=0.05):
    ...
    simple_paths = np.asarray(
        list(elem for sub in simple_path_list for elem in sub), dtype=object)
    # retrieve all simple path lengths
    simple_path_length = list(len(i) for i in list(simple_paths))
    if len(simple_path_length) == 0:
        return 0
...
def sequentiality_ratio(log):
    ...
    out_degree_events = [i[0]
                         for i in out_degree_list if i[1] == 1 or i[1] == 0]
    non_connector_nodes = list(
        set(in_degree_events).intersection(out_degree_events))
    ...
    for reference_trace in trace_transition_representation:
        for compared_trace in trace_transition_representation:
            total_overlap += jaccard_similarity(
                reference_trace, compared_trace)
