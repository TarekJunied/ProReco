...
def complexity_factor(log):
    from math import log as natural_log
    ntc, tsr, edn, ats = total_number_of_trace_classes(log), trace_similarity_rate(
        log), event_density(log), average_trace_size(log)
...
def all_block_entropy(log):
    ...
    probabilities = [(value / all_block_count)
                     for value in all_block_count_list]
    return entropy(probabilities, base=2)
