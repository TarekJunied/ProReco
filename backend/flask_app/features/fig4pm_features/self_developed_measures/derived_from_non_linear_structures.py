    ...
    for i in cuts:
        for j in cuts:
            if i is not j:
                try:
                    number_of_paths = len(
                        list(nx.node_disjoint_paths(directed_graph, i, j)))
...
def simple_path_minimum_jaccard_similarity(log, threshold=0.05):
    ...
    simple_paths = np.asarray(
        list(elem for sub in simple_paths for elem in sub), dtype=object)
    similarity = []

    # Set up progress bar
    # count number of traces and setup progress bar
    no_trace = len(simple_paths)
    progress = tqdm(total=no_trace, leave=False,
                    desc="Jaccard distance all simple paths, completed :: ")
...
def syntactic_node_similarity(log):
    ...
    for i in events:
        for j in events:
            ...
            if sim >= 0.5:
                counter += 1
    if (len(events) * (len(events) - 1) - len(events)) == 0:
        return 0
